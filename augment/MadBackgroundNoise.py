...
def _make_mad_bg_common(mad_files: list[str],
    ...
    return AddBackgroundNoise(
        mad_files, min_snr_db=min_snr_db, max_snr_db=max_snr_db, p=p, lru_cache_size=max_samples)
...
def make_mad_bg_single_class(path_to_mad,
    ...
    mad_files = [os.path.join(path_to_mad, f) for f in os.listdir(path_to_mad) if f.endswith(".wav") and mad_class.value[0] in f]
...
def make_mad_bg_exclude(path_to_mad: str,
    ...
    mad_files = [os.path.join(path_to_mad, f) for f in os.listdir(path_to_mad) if f.endswith(".wav") and class_to_exclude.value[0] not in f]
    return _make_mad_bg_common(mad_files, min_snr_db, max_snr_db, p, max_samples)
