...
from teamml_audio_augment.core.utils import get_crossfade_mask_pair
    ...
    def randomize_parameters(self, samples: torch.Tensor):
        ...
        if self.should_apply:
            if self.shift_unit == "samples":
                self.shift_amount = random.randint(
                    round(self.min_shift),
                    round(self.max_shift)
                )
            else:
    ...
    def apply(self, samples: torch.Tensor) -> torch.Tensor:
        if self.shift_unit == "samples":
            ...
            num_places_to_shift = int(
                round(self.shift_amount * samples.shape[-1])
            )
        elif self.shift_unit == "seconds":
            num_places_to_shift = int(
                round(self.shift_amount * self.sample_rate)
            )
        else:
        ...
        if self.fade:
            fade_length = int(self.sample_rate * self.fade_duration)
        ...
        LOGGER.debug(f"Applied shift of {num_places_to_shift}")
